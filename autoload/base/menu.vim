...
"""lts_build
   call add(items,{
      \ 'item' : '&LTS.&Build.&;bz\ XeLaTeX\ This\ Buffer\ (target\ =\ _buf)',
      \ 'cmd'  : 'call projs#action#bld_compile_xelatex({ "target" : "_buf" })',
      \ })

   call add(items,{
      \ 'item' : '&LTS.&Build.&;bl\ Last\ Compile\ BLD\ last_compile',
      \ 'cmd'  : 'call projs#bld#do#last_compile()',
      \ })

   call add(items,{
      \ 'item' : '&LTS.&Build.&Cleanup',
      \ 'cmd'  : 'call projs#build#cleanup()',
      \ })
...
"""lts_view
   call add(items,{
      \ 'item' : '&LTS.&View.&;bv\ Pdf\ Evince\ This\ Buffer\ (target\ =\ _buf)',
      \ 'cmd'  : printf('call projs#pdf#view({ 
        \ "target" : "%s", "type" : "%s", "viewer" : "%s" })','_buf','bld','evince'),
      \ })

   call add(items,{
      \ 'item' : '&LTS.&View.&Tex\ Today',
      \ 'cmd'  : printf('call projs#sec#open({ "target" : "%s", "type" : "%s" })','_buf','bld'),
      \ })
...
"""lts_buffer
   call add(items,{
      \ 'item' : '&LTS.&Buffer.&Database\ Record',
      \ 'cmd'  : printf('call projs#db#action("%s")','buf_data'),
      \ })

"""lts_buffer_edit
"
   call add(items,{
      \ 'item' : '&LTS.&Buffer.&Replace.&emph\ =>\ iusr',
      \ 'cmd'  : '%s/emph/iusr/g',
      \ })
...
"""lts_general
   call add(items,{
      \ 'item' : '&LTS.&General.&;gi\ \ \ Get\ Images',
      \ 'cmd'  : printf('call projs#action#get_img()'),
      \ })
