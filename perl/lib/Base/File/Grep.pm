...
Base::File::Grep - Find matches to a pattern in a series of files and related
             functions

=head1 SYNOPSIS

  use Base::File::Grep qw( fgrep fmap fdo );
...
Base::File::Grep mimics the functionality of the grep function in perl, but
...
class variable, $Base::File::Grep::SILENT to false; otherwise, no error
indication is given.
